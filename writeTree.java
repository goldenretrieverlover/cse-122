...
public class IntTree {
    ...
    private IntTreeNode writeTreeHelper(Scanner console) { // diff name same params
        if (console.hasNext()) {
            int children = console.nextInt();
            int data = console.nextInt();
            IntTreeNode root = new IntTreeNode(data);
            if (children == 1 || children == 3) {
                root.left = writeTreeHelper(console);
            }  
            if (children == 2 || children == 3) {
                root.right = writeTreeHelper(console);
            }
            return root;
        }
        return null;
